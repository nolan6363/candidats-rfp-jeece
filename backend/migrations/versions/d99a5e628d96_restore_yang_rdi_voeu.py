...
def upgrade():
    # Seed all 53 candidates (INSERT IGNORE = idempotent; safe if init.sql already ran)
    op.execute("""
        INSERT IGNORE INTO candidates (id, prenom, nom) VALUES
    ...
    # Seed all voeux (INSERT IGNORE = idempotent)
    op.execute("""
        INSERT IGNORE INTO voeux (candidate_id, rank, role) VALUES
        -- 1 Aicha Ahmed Mze Zoubeiri
...
def downgrade():
    op.execute("DELETE FROM voeux WHERE candidate_id BETWEEN 1 AND 53")
    op.execute("DELETE FROM candidates WHERE id BETWEEN 1 AND 53")
